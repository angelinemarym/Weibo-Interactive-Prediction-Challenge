...
# -*- coding: utf-8 -*-
"""
weibo_pipeline_v7.py

Ablation-guided changes from v13 (v6):
  ALPHA: 8.0 -> 12.0          (ablation: +0.004 on Jun-2015 val)
  Remove recency decay         (ablation: +0.005 on Jun-2015 val)
    comb_w = eng_w * w_content  (w_rec exponential decay term dropped)
    RECENCY_WINDOW=30d engagement boost is retained.

Ablation val scores (Jun-2015, 3k users):
  v13 baseline (ALPHA=8, w/ decay): 0.3606
  ALPHA=12 only:                    0.3646  (+0.004)
  no recency decay only:            0.3660  (+0.005)
  this version (combined):          TBD
"""
import os
import time
import math
...
RESULT_PATH    = 'Weibo Data/weibo_result_data/weibo_result_data_v7.txt'
...
def main():
    ...
    print(f"Config: no-recency-decay  rw={RECENCY_WINDOW}d  ALPHA={ALPHA}  tfidf-bigram  p5+p10")
