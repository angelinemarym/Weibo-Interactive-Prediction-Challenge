...
Change from v9: log-space candidate generation.

Percentiles and weighted average are computed in log1p space then
exponentiated back. Produces denser candidates in the low-count region
(where most posts land) instead of sampling linearly across the
heavy-tailed distribution.

v9 val score (Jun-2015, 3k users): ~0.3682
"""
import os
import re
import time
...
RECENCY_WINDOW = 30.0
MAX_CANDS      = 8
ALPHA          = 12.0  # content similarity multiplier (up from 8)
...
def build_candidates(vals, weights):
    ...
    raw = {0}
    # weighted average in log space -> geometric-mean-like centre
    raw.add(int(round(float(np.expm1(np.average(log_vals, weights=weights))))))
    # percentiles in log space -> dense near 0, sparse in tail
    for p in (5, 10, 25, 50, 75, 90):
        raw.add(int(round(float(np.expm1(np.percentile(log_vals, p))))))
...
def main():
    ...
    print(f"Config: no-recency-decay  rw={RECENCY_WINDOW}d  ALPHA={ALPHA}  tfidf-bigram  log-space-cands  +-2-refinement  strip-mentions")
